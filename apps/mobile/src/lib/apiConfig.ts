...
// In production builds, EXPO_PUBLIC_API_URL should be set to https://api.marketsignl.com
// If env var is set to localhost but we're on a production domain, override it
let API_URL = process.env.EXPO_PUBLIC_API_URL ||
  (isLocalDev ? 'http://localhost:4000' : 'https://api.marketsignl.com');
...
if (hasWindow && Platform.OS === 'web') {
  const isProductionDomain =
    hostname.includes('marketsignl.com') || hostname.includes('app.marketsignl.com') || hostname.includes('www.marketsignl.com') ||
  ...
  if (isProductionDomain && isLocalhostUrl) {
    console.warn('[API Config] Overriding localhost API URL for production domain:', API_URL, '-> https://api.marketsignl.com');
    console.warn('[API Config] Hostname:', hostname, 'Origin:', origin);
    API_URL = 'https://api.marketsignl.com';
  }
  ...
  const isProdApi = API_URL.includes('api.marketsignl.com') || API_URL.includes('api.chartsignl.com');
